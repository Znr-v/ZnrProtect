    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-3xl">
          <Feature icon={<Shield className="w-5 h-5" />} title={t("antiRaid")} desc={t("antiRaidDesc")} color="blue" />
          <Feature icon={<Zap className="w-5 h-5" />} title={t("antiPhishing")} desc={t("antiPhishingDesc")} color="yellow" />
          <Feature icon={<Lock className="w-5 h-5" />} title={t("secretScanner")} desc={t("secretScannerDesc")} color="red" />
        </div>
      </div>
    );
  }
  ...
  if (!hasAccess) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <h1 className="text-2xl font-bold mb-3">{t("unauthorizedAccess")}</h1>
        ...
        <div className="bg-theme-secondary rounded-xl p-5 border border-theme-border max-w-sm w-full text-left">
          <p className="text-xs font-semibold text-theme-muted uppercase tracking-wider mb-3">{t("whatToDo")}</p>
          <ul className="text-theme-secondary text-sm space-y-2">
            <li className="flex items-start gap-2"><span className="text-discord mt-0.5">•</span> {t("whatToDoDesc1")}</li>
            <li className="flex items-start gap-2"><span className="text-discord mt-0.5">•</span> {t("whatToDoDesc2")}</li>
          </ul>
        </div>
      </div>
    );
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-theme-secondary text-sm mt-0.5">
            {guilds.length} {guilds.length !== 1 ? t("monitoredServers") : t("monitoredServer")}
          </p>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <StatCard label={t("statServers")}        value={stats.guilds}          color="blue" />
          <StatCard label={t("statMembers")}         value={stats.members.toLocaleString("fr-FR")} />
          <StatCard label={t("statEvents24h")}    value={stats.events24h}       color={stats.events24h > 10 ? "yellow" : "green"} />
          <StatCard label={t("statOpenIncidents")} value={stats.openIncidents} color={stats.openIncidents > 0 ? "red" : "green"} />
          <StatCard label={t("statHighRiskMembers")}  value={stats.highRiskMembers} color={stats.highRiskMembers > 0 ? "red" : "green"} />
          <StatCard label={t("statPhishing7d")}   value={stats.phishingLinks}   color={stats.phishingLinks > 0 ? "yellow" : "green"} />
          <StatCard label={t("statSecrets7d")}    value={stats.secretsDetected} color={stats.secretsDetected > 0 ? "red" : "green"} />
          <StatCard label={t("statEvents7d")}     value={stats.events7d} />
        </div>
      )}

      {/* Guild list */}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-theme-primary">{t("yourServers")}</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            ...
            {g.lockdownActive && (
              <div className="bg-red-500/10 border-b border-red-500/20 px-4 py-1.5 flex items-center gap-2">
                ...
                <span className="text-red-400 text-xs font-semibold tracking-wide">{t("lockdownActive")}</span>
              </div>
            )}
            ...
            <div className="p-4">
              <Link href={`/guild/${g.id}`} className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-theme-muted text-xs mt-0.5">
                    {g._count.members} {t("members")} · {g._count.incidents} {t("incidents")}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-theme-border">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-theme-muted text-xs">{g._count.securityEvents} {t("events")}</span>
                </div>
        ...
        {guilds.length === 0 && (
          <div className="col-span-3 flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-theme-secondary font-medium">{t("noServerAccess")}</p>
            <p className="text-theme-muted text-sm mt-1">{t("contactAdminForAccess")}</p>
          </div>
        )}
      </div>
    </div>
